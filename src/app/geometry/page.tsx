...
import { BookOpen, Play, Shapes } from 'lucide-react'
import { lessons } from '@/lib/lessons'
import type { Lesson } from '@/types/lesson'

function ShapeCard({ lesson, compact = false }: { lesson: Lesson; compact?: boolean }) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 16 }}
      animate={{ opacity: 1, y: 0 }}
      whileHover={{ y: -2 }}
      className="relative bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden hover:shadow-md transition-shadow cursor-pointer"
    >
      ...
      <div className={compact ? 'p-3' : 'p-5'}>
        ...
        <h3 className={`font-bold text-slate-800 mb-1 ${compact ? 'text-sm' : 'text-base'}`}>{lesson.title}</h3>
        <p className={`text-slate-500 mb-3 line-clamp-2 ${compact ? 'text-[11px]' : 'text-xs'}`}>{lesson.description}</p>

        <div className={`bg-slate-50 rounded-lg px-3 py-1.5 mb-3 font-mono font-bold text-center ${compact ? 'text-xs' : 'text-sm'}`}
        ...
        <div className="flex gap-2">
          ...
          <Link href={`${lesson.routePath}?tab=quiz`}
            className={`flex-1 flex items-center justify-center gap-1.5 font-semibold py-2 rounded-lg bg-slate-100 text-slate-600 hover:bg-slate-200 transition-all ${compact ? 'text-xs' : 'text-sm'}`}>
  ...
  return (
    <div className="max-w-7xl mx-auto">
      ...
      <section className="mb-12">
        ...
        {/* ── Triangle Types sub-section ──────────────────────────────────── */}
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.35 }}
          className="mt-8 bg-emerald-50 border border-emerald-200 rounded-2xl p-5">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {triangleSubTypes.map((lesson, i) => (
              <motion.div key={lesson.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 + i * 0.05 }}>
